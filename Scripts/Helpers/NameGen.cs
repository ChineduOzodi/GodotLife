...
public class NameGen {
    ...
    private string GenerateName(Dictionary<string, List<char>> table, string seed = null, string start = null)
    {
        ...
        if (start == null)
        {
            s = randomStartChoice(table.Keys, randNum);
        }
        ...
        try
        {
            while (s.Length < max && s.Length < maxLen)
            {
                ...
                char n = randomChoice(table[lastSlice], randNum);
                string space = " ";
                string nString = n.ToString();
                if ( nString == space){
                    max += wordMaxLen;
                }
                s += randomChoice(table[lastSlice], randNum);
            }
        }
        catch (KeyNotFoundException) { }
    ...
    private string randomStartChoice(Dictionary<string, List<char>>.KeyCollection keys, Random random)
    {
        ...
        int actualNum = random.Next(keyList.Count);
        selectedKey = keyList[actualNum];

        return selectedKey;
    }
    private char randomChoice(List<char> charList, Random random)
    {
        char selectedChar = new Char();
        int randNum = random.Next(0, charList.Count);
