...
public class MouseClicks : Node
{
    ...
    public List<WindowDialog> dialogs;
    CanvasLayer canvas;
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        if (@event is InputEventMouseButton)
        {
            ...
            Console.WriteLine("Input mouse click");
            MouseClicked(e);
        }
    }
        ...
        if (e.ButtonIndex == (int)ButtonList.Left && e.IsPressed())
        {
            if (World.Instance != null && World.Instance.HoveredTile != null)
            {
                ...
                if (tile.cityId != null)
                {
                    ...
                    WindowDialog cityDialog = null;
                    
                    for(int i =0; i < dialogs.Count; i++)
                    {
                        WindowDialog dialog = dialogs[i];
                        if (!dialog.Visible)
                        {
                            cityDialog = dialog;
                            break;
                    ...
                    if (cityDialog == null)
                    {
                        cityDialog = (WindowDialog)dialogPrefab.Instance();
                        canvas.AddChild(cityDialog);
                        dialogs.Add(cityDialog);
                    }
                    cityDialog.WindowTitle = city.name;
                    cityDialog.PopupCentered();
